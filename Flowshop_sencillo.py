# -*- coding: utf-8 -*-
"""
Created on Sun Oct 17 17:05:26 2021
...
def FS_sen(dic):
    ...
    def R_3(model,i,j):
        if i == 1:  
            return model.c[i,j]>=model.p[i,j]
        else:
    ...
    def R_4(model,i,j):
        if i >= 2: 
            return model.c[i,j]-model.p[i,j]>=model.c[i-1,j]
        else:
            return Constraint.Skip
    ...
    def R_5(model,i,j,h,r):
        if j!=h and r< len(model.R):     
                return model.c[i,j]-model.p[i,j]+((1-model.x[j,r+1])*1900)>=model.c[i,h]-((1-model.x[h,r])*1900)
        else:    
                return Constraint.Skip
    ...
    def R_7(model,i,j):
            return model.c[i,j]== model.p[i,j]+model.s[i,j]
    model.Rest_7 = Constraint(model.I,model.J, rule=R_7)
    
    
    instance = model.create_instance(data='FSS_2X5v2.dat')
